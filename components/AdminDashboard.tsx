...
const RichTextEditor: React.FC<RichTextEditorProps> = ({ value, onChange }) => {
  ...
  useEffect(() => {
    // Only set innerHTML if different to prevent cursor jumps
    if (editorRef.current && editorRef.current.innerHTML !== value) {
        // If content is completely empty in editor, set it.
        // For updates, we usually rely on onInput.
        // However, switching posts needs this.
        editorRef.current.innerHTML = value;
    }
  }, [value]); 
...
const AdminDashboard: React.FC = () => {
  ...
  const handleEdit = async (postSummary: BlogPost) => {
    // Optimization: The 'postSummary' from the list might not have 'content' loaded.
    // Fetch full post details (with content) for editing.
    setIsSaving(true); // Reuse loading state for UI feedback
    const fullPost = await StorageService.getPostById(postSummary.id);
    setIsSaving(false);
